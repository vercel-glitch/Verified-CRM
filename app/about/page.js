...
function MissionSection({ title, paragraphs }) {
  return (
    <section className="relative pt-28 pb-20">
      <div className="absolute inset-0 -z-10 bg-gradient-to-b from-white-bg via-light-bg to-light-gray" />
      ...
      <div className="container-max">
        <div className="mx-auto max-w-3xl text-center">
          <h1 className="text-4xl md:text-5xl font-semibold tracking-tight animate-fade-up text-dark-text">{title}</h1>
          <div className="mt-6 grid gap-4">
            {paragraphs.map((p, idx) => (
              <p key={idx} className="text-base md:text-lg text-dark-text-muted animate-fade-up" style={{ animationDelay: `${120 * (idx + 1)}ms` }}>
                {p}
              </p>
            ))}
          </div>
...
function StorySection({ title, text }) {
  return (
    <section className="relative py-16 bg-white-bg">
      <div className="container-max">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 items-center">
          ...
          <div className="md:col-span-3 animate-fade-up" style={{ animationDelay: "240ms" }}>
            <div className="bg-white-bg rounded-2xl border border-light-border p-8 shadow-xl">
              <h2 className="text-2xl md:text-3xl font-semibold tracking-tight text-dark-text">{title}</h2>
              <p className="mt-4 text-base md:text-lg text-dark-text-muted">{text}</p>
            </div>
          </div>
        </div>
      </div>
...
function TeamSection({ title, members }) {
  return (
    <section className="relative py-16 bg-light-bg">
      <div className="container-max">
        <div className="mx-auto max-w-3xl text-center">
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {members.map((m, idx) => (
            <div
              key={m.name}
              className="bg-white-bg rounded-2xl border border-light-border p-6 shadow-lg hover:shadow-xl transition-transform duration-200 hover:-translate-y-0.5 animate-fade-up"
            ...
            >
              ...
              <div className="mt-4 text-center">
                ...
                <div className="text-sm text-dark-text-muted">{m.role}</div>
              </div>
            </div>
          ))}
        </div>
...
function ComplianceSection({ title, text, items = [{ label: "ISO" }, { label: "SOC 2" }, { label: "GDPR" }] }) {
  return (
    <section className="relative py-16 bg-white-bg">
      <div className="container-max">
        <div className="rounded-3xl border border-light-border bg-light-gray p-8 md:p-12 shadow-xl animate-fade-up">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-2xl md:text-3xl font-semibold tracking-tight text-dark-text">{title}</h2>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-4">
              {items.map((i) => (
                <div key={i.label} className="w-24 h-16 rounded-xl border border-light-border bg-white-bg grid place-items-center text-sm text-dark-text-muted">
                  {i.label}
                </div>
              ))}
            </div>
            <p className="mt-6 text-base md:text-lg text-dark-text-muted">{text}</p>
          </div>
        </div>
      </div>
    </section>
